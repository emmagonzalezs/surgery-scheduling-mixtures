...
def calculate_total_slack(schedule, surgery_scheduling):
    total_slack = 0
...
def calculate_total_free_time(schedule, surgery_scheduling):
    total_free_time = 0
    total_overtime = 0
    for or_id, or_surgeries in schedule.items():
...
def run_first_fit(surgery_scheduling):
    start_time3 = time.time()
    ff_schedule, ff_overtime = surgery_scheduling.first_fit()
    end_time3 = time.time()
    ff_time = end_time3 - start_time3
    print("ff finished, took:", ff_time)
    total_slack_ff = calculate_total_slack(ff_schedule, surgery_scheduling)
    total_free_time_ff = calculate_total_free_time(ff_schedule, surgery_scheduling)
    return ff_schedule, ff_overtime, ff_time, total_slack_ff, total_free_time_ff


def run_lpt(surgery_scheduling):
    ...
    total_slack_lpt = calculate_total_slack(lpt_schedule, surgery_scheduling)
    total_free_time_lpt = calculate_total_free_time(lpt_schedule, surgery_scheduling)
    return lpt_schedule, overtime_lpt, lpt_time, total_slack_lpt, total_free_time_lpt, free_time_lpt


def run_regret_based(surgery_scheduling, z, samples):
    start_time1 = time.time()
    best_schedule_output, best_schedule_overtime_output = surgery_scheduling.regret_based_sampling(z=z, samples=samples)
    end_time1 = time.time()
    regret_based_sampling_time = end_time1 - start_time1
    print('regret finished, took:', regret_based_sampling_time)
    total_slack_regret_based = calculate_total_slack(best_schedule_output, surgery_scheduling)
    total_free_time_regret_based = calculate_total_free_time(best_schedule_output, surgery_scheduling)
    return best_schedule_output, best_schedule_overtime_output, regret_based_sampling_time, total_slack_regret_based, total_free_time_regret_based
...
if __name__ == '__main__':
    ...
    with Pool(cpu_count()) as pool:
        # Run first_fit in a separate process
        ff_results = pool.apply_async(run_first_fit, (surgery_scheduling,))

        # Run lpt in a separate process
        lpt_results = pool.apply_async(run_lpt, (surgery_scheduling,))

        # Run regret_based_sampling in a separate process
        regret_results = pool.apply_async(run_regret_based, (surgery_scheduling, 4, 1))

        # Get results
        ff_schedule, ff_overtime, ff_time, total_slack_ff, total_free_time_ff = ff_results.get()
        lpt_schedule, overtime_lpt, lpt_time, total_slack_lpt, total_free_time_lpt, free_time_lpt = lpt_results.get()
        best_schedule_output, best_schedule_overtime_output, regret_based_sampling_time, total_slack_regret_based, total_free_time_regret_based = regret_results.get()
